...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    private List<MarkerAndShape> daftarMarkerDanShape = new ArrayList<MarkerAndShape>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        daftarMarkerDanShape.add(new MarkerAndShape("Marker", "Universitas Multimedia Nusantara", 0.0, lokalTitiks));
        daftarMarkerDanShape.add(new MarkerAndShape("Circle", "Area 500 m dari UMN", 500.0, lokalTitiks));
        lokalTitiks.clear();
        ...
        daftarMarkerDanShape.add(new MarkerAndShape("PolyLine", "Jalur UMN ke Bethsaida", 0.0, lokalTitiks));
        lokalTitiks.clear();
        ...
        daftarMarkerDanShape.add(new MarkerAndShape("Polygon", "Area Kampus UMN", 0.0, lokalTitiks));
        lokalTitiks.clear();
    }

    protected void gambarMarkerDanShape() {
        for (int i = 0; i < daftarMarkerDanShape.size(); i++) {
            MarkerAndShape ms = daftarMarkerDanShape.get(i);
            if (ms.getJenis().equalsIgnoreCase("Marker")) {
                ...
                ms.setObjekPeta(mMap.addCircle(new CircleOptions().center(ms.getTitiks().get(0)).radius(ms.getRadius()).strokeColor(Color.YELLOW).fillColor(Color.argb(30, 255, 255, 0))));
                ...
                pg.add(ms.getTitiks().get(0)).strokeColor(Color.BLUE).strokeWidth(10.0f).fillColor(Color.argb(20, 0, 255, 255));
    ...
    protected void onActivityResult(int reqCode, int resultCode, @Nullable Intent data) {
        ...
        if (resultCode == RESULT_OK && reqCode == REQUEST_TAMBAH) {
            ...
            MarkerAndShape mds = new MarkerAndShape(jenis, keterangan,
                    radius, mTitiks);
